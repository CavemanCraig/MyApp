...
public class MyWebService {
	...
	// If no player with that ID exists, we return 100 (the default for a new player)
	@GET
	@Path("/PlayerPoints/{facebookID}")
	@Produces("application/json")
	public long getPlayerPoints(@PathParam("facebookID") long facebookID) {
		System.out.println("GET on specific Player's Points for Facebook User ID: ["
				+ facebookID + "]");
		...
		if (null == foundPlayer) {
			...
			System.out.println("Returning his points of: " + foundPlayer.getPoints());
